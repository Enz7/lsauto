...
import { Search, MapPin, CheckCircle2, MessageCircle, Star, Briefcase, FileCheck, ChevronRight, Loader2 } from 'lucide-react';
...
const MapRussia = ({ suppliers, onSelect }: { suppliers: Поставщик[]; onSelect: (city: string) => void }) => {
  const supplierCities = suppliers.map(s => s.город);
  return (
    <div className="rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl" style={{ height: 'clamp(260px, 50vw, 480px)' }}>
      ...
      >
        ...
        {Object.entries(cityCoords).map(([city, pos]) => {
          ...
          const citySuppliers = suppliers.filter(s => s.город === city);
          return (
            <Marker key={city} position={[pos.lat, pos.lng]} icon={createMarkerIcon(hasSupplier)}>
...
export const Suppliers = () => {
  ...
  const [suppliers, setSuppliers] = useState<Поставщик[]>([]);
  ...
  return (
    ...
    <div className="space-y-12">
      ...
      {viewType === 'map' ? (
        <MapRussia suppliers={suppliers} onSelect={(city) => navigate(`/city/${city}`)} />
      ) : (
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-between">
                <span className="text-[10px] text-gray-600 font-black">
                  {suppliers.filter(s => s.город === city).length || cityCoords[city].count} ПОСТАВЩИКОВ
                </span>
            ...
            <div key={supplier.id} className="bg-dark-card border border-white/5 rounded-2xl overflow-hidden flex flex-col group">
              ...
              <div className="p-6 space-y-4 flex-grow">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-gray-400 text-sm mt-1">
                      <MapPin size={14} /> {supplier.город || 'Не указан'}
                    </div>
                  </div>
                ...
                <p className="text-sm text-gray-400 line-clamp-3">
                  {supplier.описание || 'Поставщик автомобилей на платформе LSAuto.'}
                </p>
