...
export const Favorites = () => {
  const { favorites, toggleFavorite, allCars, clearFavorites } = useApp();
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-bold mb-2 uppercase tracking-tighter">Избранное</h1>
          <p className="text-gray-400 text-sm">Сохранено {favoriteCars.length} автомобиля</p>
        </div>
        <button 
          onClick={() => { if(confirm('Очистить весь список?')) clearFavorites(); }}
          className="text-xs font-bold text-red-500 hover:bg-red-500/10 px-4 py-2 rounded-xl transition-all"
        >
          Очистить всё
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {favoriteCars.map(car => (
          <div key={car.id} className="bg-dark-card border border-white/5 rounded-3xl overflow-hidden group relative animate-in zoom-in-95 duration-300">
            <button 
